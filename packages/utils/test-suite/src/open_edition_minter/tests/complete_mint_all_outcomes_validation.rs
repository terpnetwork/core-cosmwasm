...
fn check_mint_revenues_distribution() {
    ...
    assert_eq!(
        ...
        "IncorrectPaymentAmount 100uterp != 100000000uterp"
    );
    ...
    assert_eq!(
        res.err().unwrap().source().unwrap().to_string(),
        "IncorrectPaymentAmount 200000000uterp != 100000000uterp"
    );
    ...
    assert_eq!(
        res.err().unwrap().source().unwrap().to_string(),
        "IncorrectPaymentAmount 0uterp != 100000000uterp"
    );
    ...
    // Creator should be at +100 x2 thiols - mint fees (currently at 10 x2) [Mint fees include Dev fees]
