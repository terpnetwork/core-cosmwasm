...
const CONTRACT_NAME: &str = "crates.io:terp-fair-burn";
            ...
            if let Some(dist_coin) = dist_coin {
                match &recipient {
                    ...
                    None => {
                        ...
                        response =
                            response.add_message(create_fund_community_pool_msg(vec![dist_coin]));
                    }
                }
            }
    ...
    for (entry_key, funds) in payout_map {
        match entry_key {
            k if k == fair_burn_pool_key => {
                ...
                response = response
                    .add_event(event)
                    .add_message(create_fund_community_pool_msg(funds));
            }
            k => {
