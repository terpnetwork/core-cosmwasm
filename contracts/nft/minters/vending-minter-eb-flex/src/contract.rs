...
// Check earlybird per limit address
...
fn is_public_mint(deps: Deps, info: &MessageInfo) -> Result<bool, ContractError> {
    ...
    let wl_mint_count = earlybird_mint_count(deps, info)?;
    let wl_limit: Member = deps.querier.query_wasm_smart(
        earlybird,
        &EarlybirdQueryMsg::Member {
            member: info.sender.to_string(),
        },
    )?;
...
fn _execute_mint(
    ...
    if is_public {
        ...
        // Save the new mint count for the sender's address
        let new_mint_count = earlybird_mint_count(deps.as_ref(), &info)? + 1;
...
fn earlybird_mint_count(deps: Deps, info: &MessageInfo) -> Result<u32, StdError> {
