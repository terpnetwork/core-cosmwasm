...
use semver::Version;
use terp721::InstantiateMsg;
use terp721_base::msg::CollectionInfoResponse;
...
use terp721_base::ContractError::Unauthorized;
use terp721_base::Terp721Contract;
pub type Terp721UpdatableContract<'a> = Terp721Contract<'a, Extension>;
use terp_sdk::Response;

const CONTRACT_NAME: &str = "crates.io:terp721-updatable";
const CONTRACT_VERSION: &str = env!("CARGO_PKG_VERSION");
const EARLIEST_COMPATIBLE_VERSION: &str = "0.16.0";
const COMPATIBLE_CONTRACT_NAMES_FOR_MIGRATION: [&str; 4] = [
    "terp721-base",
    "crates.io:terp721-base",
    "terp721-updatable",
    "crates.io:terp721-updatable",
];
...
pub fn _instantiate(
    ...
    let res = Terp721UpdatableContract::default().instantiate(deps, env, info, msg)?;
    Ok(res)
}
...
pub fn execute_enable_updatable(
    ...
    let collection_info: CollectionInfoResponse =
        Terp721UpdatableContract::default().query_collection_info(deps.as_ref())?;
...
pub fn execute_update_token_metadata(
    ...
    Terp721UpdatableContract::default().tokens.update(
        deps.storage,
        &token_id,
        |token| match token {
            Some(mut token_info) => {
...
pub fn _migrate(mut deps: DepsMut, env: Env, _msg: Empty) -> Result<Response, ContractError> {
    ...
    if ["terp721-base", "crates.io:terp721-base"].contains(&prev_contract_name.as_str()) {
        // if migrating from terp721-base, initialize flags
        FROZEN_TOKEN_METADATA.save(deps.storage, &false)?;
    ...
    if prev_contract_version < Version::new(3, 0, 0) {
        response = terp721_base::upgrades::v3_0_0::upgrade(deps.branch(), &env, response)?;
    }

    #[allow(clippy::cmp_owned)]
    if prev_contract_version < Version::new(3, 1, 0) {
        response = terp721_base::upgrades::v3_1_0::upgrade(deps.branch(), &env, response)?;
    }
...
mod tests {
    ...
    use cosmwasm_std::{
         to_json_binary, ContractInfoResponse, ContractResult, Empty, OwnedDeps, Querier,
        QuerierResult, QueryRequest, SystemError, SystemResult, WasmQuery, from_json,
    };
    ...
    fn update_token_metadata() {
        ...
        let contract = Terp721UpdatableContract::default();
