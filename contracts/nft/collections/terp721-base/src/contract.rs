...
use terp721::{
    CollectionInfo, ExecuteMsg, InstantiateMsg, ResidualInfo, ResidualInfoResponse,
    UpdateCollectionInfoMsg,
};
use terp_sdk::Response;

use crate::msg::{CollectionInfoResponse, NftParams, QueryMsg};
use crate::{ContractError, Terp721Contract};
    ...
    T: Serialize + DeserializeOwned + Clone,
{
    pub fn instantiate(
        ...
        let residual_info: Option<ResidualInfo> = match msg.collection_info.residual_info {
            Some(residual_info) => Some(ResidualInfo {
                payment_address: deps.api.addr_validate(&residual_info.payment_address)?,
                share: share_validate(residual_info.share)?,
            }),
            None => None,
        };
    ...
    pub fn execute(
        ...
        match msg {
            ...
            terp721::ExecuteMsg::UpdateOwnership(msg) => self
                .parent
                .execute(
                    deps,
                    env,
    ...
    pub fn update_collection_info(
        ...
        env: Env,
        info: MessageInfo,
        collection_msg: UpdateCollectionInfoMsg<ResidualInfoResponse>,
    ) -> Result<Response, ContractError> {
        ...
        if let Some(Some(new_royalty_info_response)) = collection_msg.residual_info {
            ...
            let new_royalty_info = ResidualInfo {
            ...
            if let Some(old_royalty_info) = collection.residual_info {
            ...
            collection.residual_info = Some(new_royalty_info);
            self.royalty_updated_at
                .save(deps.storage, &env.block.time)?;
        }
    ...
    pub fn query_collection_info(&self, deps: Deps) -> StdResult<CollectionInfoResponse> {
        ...
        let royalty_info_res: Option<ResidualInfoResponse> = match info.residual_info {
            Some(residual_info) => Some(ResidualInfoResponse {
                payment_address: residual_info.payment_address.to_string(),
        ...
        Ok(CollectionInfoResponse {
            ...
            residual_info: royalty_info_res,
        })
    }
