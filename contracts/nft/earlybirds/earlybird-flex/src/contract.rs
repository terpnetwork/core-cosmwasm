...
pub fn instantiate(
    ...
    for member in msg.members.into_iter() {
        ...
        EARLYBIRD.save(deps.storage, addr, &member.mint_count)?;
    }
...
pub fn execute_update_start_time(
    ...
    // don't allow updating start time if earlybird is active
...
pub fn execute_update_end_time(
    ...
    // if earlybird already started don't allow updating end_time unless
    // it is to reduce it
...
pub fn execute_add_members(
    ...
    for add in msg.to_add.into_iter() {
        ...
        EARLYBIRD.save(deps.storage, addr, &add.mint_count)?;
...
pub fn query_member(deps: Deps, member: String) -> StdResult<Member> {
    let addr = deps.api.addr_validate(&member)?;
    let mint_count = EARLYBIRD.load(deps.storage, addr.clone())?;
    Ok(Member {
        address: addr.into_string(),
        mint_count,
    })
