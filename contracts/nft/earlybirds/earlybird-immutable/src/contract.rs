...
pub fn instantiate(
    ...
    msg.addresses.dedup();
    let count = update_earlybird(&mut deps, msg)?;
    validate_nonempty_earlybird(count)?;
...
fn validate_nonempty_earlybird(count: u64) -> Result<bool, ContractError> {
    if count < 1 {
        return Err(ContractError::EmptyEarlybird {});
    }
    Ok(true)
}
...
pub fn query(deps: Deps, _env: Env, msg: QueryMsg) -> StdResult<Binary> {
    match msg {
        ...
        QueryMsg::IncludesAddress { address } => to_json_binary(&query_includes_address(deps, address)?),
