...
use crate::headstash_airdrop::constants::collection_constants::WHITELIST_AMOUNT;
use crate::headstash_airdrop::setup::configure_mock_minter::configure_mock_minter_with_mock_earlybird;
use crate::headstash_airdrop::setup::setup_signatures::{
...
use headstash_airdrop::msg::{ExecuteMsg, QueryMsg};
...
use crate::headstash_airdrop::constants::claim_constants::{
    ...
    TERP_WALLET_01, TERP_WALLET_02,
};

use crate::headstash_airdrop::setup::execute_msg::{
...
fn query_minter_as_expected(app: &mut TerpApp, airdrop_contract: Addr, minter_addr: Addr) {
...
fn test_airdrop_eligible_query() {
    ...
    let (_, _, _, eth_addr_str) = get_wallet_and_sig(claim_plaintext.clone());
    let mut app = custom_mock_app();
    configure_mock_minter_with_mock_earlybird(&mut app);
...
fn test_claim_one_valid_airdrop() {
    ...
    let balances = app
        .wrap()
        .query_all_balances(terp_wallet_01.clone())
        .unwrap();
    assert_eq!(balances, []);
    ...
    let balances = app.wrap().query_all_balances(terp_wallet_01).unwrap();
...
fn test_ineligible_does_not_receive_funds() {
    ...
    instantiate_contract(params).unwrap();
    query_minter_as_expected(&mut app, airdrop_contract.clone(), minter_addr);
    let terp_wallet_02 = Addr::unchecked(TERP_WALLET_02);
    let balances = app
        .wrap()
        .query_all_balances(terp_wallet_02.clone())
        .unwrap();
    assert_eq!(balances, []);

    let claim_plaintext_2 = &get_msg_plaintext(TERP_WALLET_02.to_string());
    ...
    let res = execute_contract_error_with_msg(
        claim_message,
        &mut app,
        terp_wallet_02.clone(),
        airdrop_contract,
    );
    let expected_error = format!("Address {eth_addr_str_2} is not eligible");
    assert_eq!(res, expected_error);
    let balances = app.wrap().query_all_balances(terp_wallet_02).unwrap();
    let expected_balance = [];
    assert_eq!(balances, expected_balance)
}

#[test]
fn test_one_eth_claim_two_terp_addresses_invalid() {
    let wallet_1 = LocalWallet::new(&mut thread_rng());
    let terp_wallet_01 = Addr::unchecked(TERP_WALLET_01);
    let terp_wallet_02 = Addr::unchecked(TERP_WALLET_02);

    let claim_plaintext_1 = &get_msg_plaintext(TERP_WALLET_01.to_string());
    ...
    // claim with eth address 1, terp wallet 1
    let claim_message = ExecuteMsg::ClaimAirdrop {
    ...
    // claim with eth address 1, terp wallet 2
    let claim_message = ExecuteMsg::ClaimAirdrop {
        eth_address: eth_addr_str_1.clone(),
        eth_sig: eth_sig_str_2,
    };
