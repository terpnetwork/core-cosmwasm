...
use crate::common_setup::contract_boxes::{contract_collection_earlybird, custom_mock_app};
...
fn instantiate_contract(admin_account: &str, app: &mut TerpApp) {
    ...
    let collection_id = app.store_code(contract_collection_earlybird());
    let _ = app.instantiate_contract(
        collection_id,
        admin,
        &msg,
        &coins(funds_amount, NATIVE_DENOM),
        "collection_earlybird".to_string(),
        None,
    );
}

fn add_members_with_specified_admin(admin: &str, members: Vec<String>, app: &mut TerpApp) {
    let admin_addr = Addr::unchecked(admin);
    let collection_earlybird_contract = Addr::unchecked("contract0");
    let initial_members = vec!["member0".to_string()];
    ...
    let query_result: MembersResponse = app
        .wrap()
        .query_wasm_smart(collection_earlybird_contract.clone(), &query_msg)
        .unwrap();
    ...
    let res = app.execute_contract(admin_addr, collection_earlybird_contract.clone(), &msg, &[]);
    ...
    let query_result: MembersResponse = app
        .wrap()
        .query_wasm_smart(collection_earlybird_contract, &query_msg)
        .unwrap();
    assert_eq!(query_result, expected_result);
}

fn remove_members_with_specified_admin(admin: &str, members: Vec<String>, app: &mut TerpApp) {
    let admin_addr = Addr::unchecked(admin);
    let collection_earlybird_contract = Addr::unchecked("contract0");
...
fn add_members_blocked(admin: &str, members: Vec<String>, app: &mut TerpApp) {
    let admin_addr = Addr::unchecked(admin);
    let collection_earlybird_contract = Addr::unchecked("contract0");
    let initial_members = vec!["member0".to_string()];
...
fn test_add_admin() {
    ...
    let collection_earlybird_addr = Addr::unchecked(COLLECTION_WHITELIST_ADDR);
    ...
    let _ = app.execute_contract(
        ...
        Addr::unchecked(collection_earlybird_addr),
...
fn test_query_admin_list() {
    let mut app = custom_mock_app();
    instantiate_contract(ADMIN, &mut app);
    let collection_earlybird_contract = Addr::unchecked(COLLECTION_WHITELIST_ADDR);
...
fn test_freeze_admins() {
    ...
    let _ = app.execute_contract(
        Addr::unchecked(ADMIN),
        Addr::unchecked(collection_earlybird_addr.clone()),
        &freeze_admins_msg,
        &[],
    );
