...
fn setup_contract(deps: DepsMut) {
    ...
    let info = mock_info(ADMIN, &[coin(100_000_000, "uterp")]);
...
fn not_uterp_denom() {
    ...
    let msg = InstantiateMsg {
        ...
        start_time: END_TIME,
        end_time: END_TIME,
        mint_price: coin(UNIT_AMOUNT, "not_uterp"),
        per_address_limit: 1,
        member_limit: 1000,
...
fn improper_initialization_invalid_creation_fee() {
    ...
    let msg = InstantiateMsg {
        ...
        start_time: END_TIME,
        end_time: END_TIME,
        mint_price: coin(UNIT_AMOUNT, "uterp"),
        per_address_limit: 1,
        member_limit: 3000,
        admins: vec![ADMIN.to_string()],
        admins_mutable: true,
...
fn increase_member_limit() {
    ...
    let info = mock_info(ADMIN, &[coin(0, "uterp")]);
    let res = execute(deps.as_mut(), mock_env(), info, msg);
    assert!(res.is_ok());
    ...
    let msg = ExecuteMsg::IncreaseMemberLimit(1002);
    let info = mock_info(ADMIN, &[coin(1, "uterp")]);
    execute(deps.as_mut(), mock_env(), info, msg).unwrap_err();
    ...
    let info = mock_info(ADMIN, &[coin(200_000_000, "uterp")]);
    let res = execute(deps.as_mut(), mock_env(), info, msg);
    assert!(res.is_ok());

    // over MAX_MEMBERS, Invalid member limit
    let msg = ExecuteMsg::IncreaseMemberLimit(6000);
    let info = mock_info(ADMIN, &[coin(400_000_000, "uterp")]);
    let err = execute(deps.as_mut(), mock_env(), info, msg).unwrap_err();
    assert_eq!(
        err.to_string(),
