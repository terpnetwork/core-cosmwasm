/**
 * This file was automatically generated by @cosmwasm/ts-codegen@0.25.2.
 * DO NOT MODIFY IT BY HAND. Instead, modify the source JSONSchema file,
 * and run the @cosmwasm/ts-codegen generate command to regenerate this file.
 */

export type Uint128 = string
export interface InstantiateMsg {
  params: MinterParamsForParamsExtension
}
export interface MinterParamsForParamsExtension {
  allowed_terp721_code_ids: number[]
  code_id: number
  creation_fee: Coin
  extension: ParamsExtension
  frozen: boolean
  max_trading_offset_secs: number
  min_mint_price: Coin
  mint_fee_bps: number
}
export interface Coin {
  amount: Uint128
  denom: string
  [k: string]: unknown
}
export interface ParamsExtension {
  airdrop_mint_fee_bps: number
  airdrop_mint_price: Coin
  max_per_address_limit: number
  max_token_limit: number
  shuffle_fee: Coin
}
export type FactoryUtilsExecuteMsgForVendingMinterInitMsgExtension = {
  create_minter: CreateMinterMsgForVendingMinterInitMsgExtension
}
export type Decimal = string
export type Timestamp = Uint64
export type Uint64 = string
export interface CreateMinterMsgForVendingMinterInitMsgExtension {
  collection_params: CollectionParams
  init_msg: VendingMinterInitMsgExtension
}
export interface CollectionParams {
  code_id: number
  info: CollectionInfoForResidualInfoResponse
  name: string
  symbol: string
}
export interface CollectionInfoForResidualInfoResponse {
  creator: string
  description: string
  explicit_content?: boolean | null
  external_link?: string | null
  image: string
  residual_info?: ResidualInfoResponse | null
  start_trading_time?: Timestamp | null
}
export interface ResidualInfoResponse {
  payment_address: string
  share: Decimal
}
export interface VendingMinterInitMsgExtension {
  base_token_uri: string
  mint_price: Coin
  num_tokens: number
  payment_address?: string | null
  per_address_limit: number
  start_time: Timestamp
  earlybird?: string | null
}
export type FactoryUtilsQueryMsg =
  | {
      params: {}
    }
  | {
      allowed_collection_code_ids: {}
    }
  | {
      allowed_collection_code_id: number
    }
