...
import { InstantiateMsg as ResidualRegistryInstantiateMsg } from '@terpnetwork/core-cosmwasm-types/src/ResidualRegistry.types'
import { InstantiateMsg as VendingFactoryInstantiateMsg } from '@terpnetwork/core-cosmwasm/src/VendingFactory.types'
import fs from 'fs'
...
export const CONTRACT_MAP = {
  // core artifacts
  FAIR_BURN: 'terp_fair_burn',
  ROYALTY_REGISTRY: 'terp_residual_registry',
...
export default class Context {
  ...
  instantiateContracts = async () => {
    ...
    // Instantiate terp_fair_burn
    let instantiateFairBurnResult = await this.instantiateContract(client, sender, CONTRACT_MAP.FAIR_BURN, {
      fee_bps: 5000,
    })

    // Instantiate terp_residual_registry
    let residualRegistryInstantiateMsg: ResidualRegistryInstantiateMsg = {
      config: {
        max_share_delta: '0.10',
        update_wait_period: 12,
      },
    }
    await this.instantiateContract(client, sender, CONTRACT_MAP.ROYALTY_REGISTRY, residualRegistryInstantiateMsg)
    ...
    let vendingFactoryInstantiateMsg: VendingFactoryInstantiateMsg = {
      params: {
        ...
        min_mint_price: { amount: '1000000', denom: 'uterp' },
        mint_fee_bps: 200,
        extension: {
          airdrop_mint_fee_bps: 200,
          airdrop_mint_price: { amount: '1000000', denom: 'uterp' },
          max_per_address_limit: 10_000,
          max_token_limit: 10_000,
          shuffle_fee: { amount: '1000000', denom: 'uterp' },
        },
      },
    }
